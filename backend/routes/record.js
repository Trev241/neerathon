...
recordRoutes.route("/record").get(jwtCheck, async (req, res) => {
  ...
  const records = await db_connect.collection("Participant").find({}).toArray();
...
recordRoutes
  .route("/record/add")
  .post(upload.single("paymentAttachment"), async (req, response, error) => {
    if (suspendAdditions) {
    ...
    await mega_connect.upload(filename, Buffer.from(file.buffer, "base64"))
      .complete;
